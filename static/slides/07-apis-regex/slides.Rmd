...
We will request information regarding genes that are annotated with the go term [GO:0140374](http://amigo.geneontology.org/amigo/term/GO:0140374) (antiviral innate immune response), that have evidence code [ECO:0000006](https://evidenceontology.org/browse/#ECO_0000006) (experimental evidence)

```{r example-gene-ontology, cache=TRUE}
go_query <- GET(
  # In this case, a parameter of the query is passed directly in the URL
  url   = "http://api.geneontology.org/api/bioentity/function/GO:0140374/genes",
  query = list(
    evidence          = "ECO:0000006",
    relationship_type = "involved_in"
  ),
  # May need to pass this option to curl to allow to wait 
  config = config(
    connecttimeout = 60
    )
)
...
## API Example 2: Using a Token (cont. 1)
