...
init <- function(nsick, nwear, pop_size, nsteps) {
  ...
  # Sampling sick and which wears/doesn't wear a mask
  status_prev[sample.int(n = pop_size, size = nsick, replace = FALSE)] <- codes["sick"]
...
update_status <- function(i, j, data.) {
  ...
  if (data.$status_prev[i, j] %in% codes[c("deseased", "recovered")])
    return()
  
  # Is it getting better?
  if (data.$status_prev[i, j] == codes["sick"]) {
    ...
    # Each sick neighbor is an independent chance of getting sick
    local <- local[which(data.$status_prev[local] == codes["sick"]), drop=FALSE]
    
    # No neighbor sick, nothing happens
    if (length(local) == 0L)
      return()
