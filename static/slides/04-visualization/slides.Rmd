...
Note we have to deal with the `NA` category.
...
```{r warning=FALSE, out.width='30%', message=FALSE}
ggplot(data = met_avg, mapping = aes(x = temp, y = rh, alpha = 0.5)) + 
...
# Coloring barcharts
More interestingly, you can fill by another variable (here, 'clarity'). We also show that we can change the color scale.
...
bar <- ggplot(data = met_avg) + 
  geom_bar(mapping = aes(x = elev_cat, fill = elev_cat), 
