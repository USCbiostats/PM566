...
- Both `plot_ly()` and `ggplotly()` line plots can be specified with little code
  - Compare the `tooltip` results for both plots

```{r, eval=FALSE}

g1 = plot_ly(cv_states, x = ~date, y = ~deaths, color = ~state, type = "scatter", mode = "lines")
g1 = ggplot(cv_states, aes(x = date, y = cases, color = state)) + geom_line() + geom_point(size = .5, alpha = 0.5)
ggplotly(g1)
...
## Line graph: `plot_ly()` vs. `ggplotly()` NEED TO ARRANGE THESE
