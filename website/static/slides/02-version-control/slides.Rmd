...
c. Push the changes to the remote using `git branch` and `git push` like this:
  ...
  what git tells you about it. Origin is the tag associated with the remote repo you set up, while main is the tag associated with the current branch of
  your repo.
...
every time you want to push (and pull, if private).  When asked for your password, GitHub actually wants your Personal access token.
...
For the first `git push`, you need to specify source (main) and target
